...
| `groupBy`        | `group-by`         | Using this parameter, the table rows can bee grouped based on column ids A multiple step grouping is possible by using a ";" between groups Example: hid-1,hid-3 OR hid-1,hid-3;hid-2 Note: if value is "", remove any grouping | `string` | `undefined`                                           |
| `orderBy`        | `order-by`         | This is used to sort the table depending of a column The column cells must be single values Note: if value is "", remove any ordering                                                                                           | `string` | `undefined`                                           |
